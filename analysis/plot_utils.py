...
def volcano_plot(x, y, row, gene_names, fontsize=12, gene_fontsize=12):
    selected_genes = row['Lead_genes'].split(';')
    m = gene_names.isin(selected_genes)
    idxs = np.argwhere(m).ravel()
    
    plt.title(row['Term'], fontsize=fontsize)
    ...
    for i in idxs:
        ...
        texts.append(plt.text(x[i], y[i], txt, fontsize=gene_fontsize))
...
class DotPlot(object):
    ...
    def process(self, df: pd.DataFrame):
        ...
        if (
            ...
            df = (
                df.groupby(self.x)
                .apply(lambda _x: _x.sort_values(by=self.colname, descending=True).head(self.n_terms))
                # .apply(lambda _x: _x.abs().sort_values(by='NES', descending=True).head(self.n_terms))
                .reset_index(drop=True)
            )
        else:
            # df = df.sort_values(by=self.colname).tail(self.n_terms)  # ascending
            # df = df.sort_values(by='abs_NES').tail(self.n_terms)  # ascending
            df = df.head(self.n_terms)
    ...
    def add_colorbar(self, sc):
        ...
        cbar.ax.yaxis.set_tick_params(
            color="white", direction="in", left=True, right=True, labelsize=14
        )
        cbar.ax.set_title(self.cbar_title, loc="left", fontweight="bold", fontsize=16)
        for key, spine in cbar.ax.spines.items():
            spine.set_visible(False)
