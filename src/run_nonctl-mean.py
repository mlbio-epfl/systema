...
if __name__ == '__main__':
    ...
    for condition in tqdm(unique_conds):
        ...
        # Non-ctl mean
        results_df.loc[len(results_df)] = ['non-ctl mean',
                                           condition,
                                           pearsonr(delta_true, delta_pert)[0],
                                           pearsonr(delta_true[top20_de_idxs], delta_pert[top20_de_idxs])[0],
                                           one_gene_str,
                                           n_train]

    results_df.to_csv(f'{args.outdir}/{args.dataset}_{args.seed}_nonctl-mean_results.csv', index=False)